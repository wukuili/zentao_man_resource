...
(
    set::current($status),
    set::linkParams("projectID={$projectID}&status={key}&begin=" . str_replace('-', '_', $begin) . "&end=" . str_replace('-', '_', $end) . "&users={$users}&showHoliday={$showHoliday}&execution={$execution}&viewType={$viewType}"),
    to::item($navItems)
);
...
if(common::hasPriv('man_resource', 'exportProject'))
{
    ...
    (
        ...
        set::url(helper::createLink('man_resource', 'exportProject', "projectID={$projectID}&begin=" . strtotime($begin) . "&end=" . strtotime($end) . "&mode={$status}"))
    );
}

if($execution > 0 && common::hasPriv('task', 'create'))
{
    $toolbarItems[] = btn
    (
        set::type('ghost'),
        set::icon('plus'),
        set::text($lang->man_resource->createTask),
        set::url(helper::createLink('task', 'create', "executionID={$execution}"))
    );
}
...
if(!empty($calendarData))
{
    foreach($calendarData as $userID => $data)
    {
        ...
        $row = new stdClass();
        $row->id               = $userID;
        $memberBegin           = str_replace('-', '_', $begin);
        $memberEnd             = str_replace('-', '_', $end);
        $row->realname         = html::a(createLink('man_resource', 'memberdimension', "userID=$userID&status=$status&begin=$memberBegin&end=$memberEnd&showHoliday=$showHoliday&execution=$execution&projectID=$projectID"), $data['realname']);
